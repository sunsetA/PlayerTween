...
namespace RopeMinikit
{
    public class RopeMouseInteraction : MonoBehaviour
    {
        ...
        public void FixedUpdate()
        {
            if (isExtra)
            {
                //Ray ray1 = Camera.main.ScreenPointToRay(Camera.main.WorldToScreenPoint(RayPoint.position));
                //var closestRope = GetClosestRope(ray1, out int closestParticleIndex, out float closestDistanceAlongRay);
                pulledRope = ropes[ropes.Count - 1];
                pulledRope.GetClosestParticle(RayPoint.transform.position, out pulledParticle, out pulledDistance);

                pulledParticle = /*pulledRope.positions.Length*/0;
                //pulledParticle = pulledParticle;
                //pulledDistance = closestDistanceAlongRay;

                currentPosition = pulledRope.GetPositionAt(pulledParticle);
                targetPosition = RayPoint.transform.position;
